...
class Faturamento_model extends CI_Model{
    ...
    public function retornar_clientes_faturamento($dia_faturamento, $competencia=null){
        return $this->db->query('SELECT * FROM cliente WHERE consultor = 0 AND '.$this->sql_dia_vencimento_cliente($dia_faturamento, $competencia).' AND id_franquia_fk = 0 AND status = 1 AND id_cliente>4 ORDER BY nome_ou_fantasia ASC');
    }

    public function retornar_clientes_faturamento_franquia($dia_faturamento,$franquia = 0,$competencia=null){
		$sql = 'SELECT * FROM cliente WHERE consultor = 0 AND '.$this->sql_dia_vencimento_cliente($dia_faturamento, $competencia).' AND id_franquia_fk = '.$franquia.' AND status = 1 AND id_cliente>4 ORDER BY nome_ou_fantasia ASC';
        return $this->db->query($sql);
    }

    public function retornar_gerar_faturamento_consultas($inicio_faturamento,$fim_faturamento,$dia_vencimento,$competencia=null){
        $sql  = 'SELECT tbmain.* FROM consulta_efetuada AS tbmain ';
        $sql .= 'LEFT JOIN cliente AS tb2 ON tbmain.id_cliente_fk = tb2.id_cliente ';
        $sql .= 'WHERE (tbmain.criado_em BETWEEN "'.$inicio_faturamento.'" AND "'.$fim_faturamento.'") AND tbmain.id_cliente_fk > 4 AND '.$this->sql_dia_vencimento_cliente($dia_vencimento, $competencia, 'tb2').' AND tb2.status = 1 ';
    ...
    public function retornar_gerar_faturamento($inicio_faturamento,$fim_faturamento,$dia_vencimento,$competencia=null){
        ...
        $sql .= ') AS tbmain ';
        $sql .= 'LEFT JOIN cliente AS tb2 ON tbmain.id_cliente_fk = tb2.id_cliente ';
        $sql .= 'WHERE tbmain.id_cliente_fk > 4 AND '.$this->sql_dia_vencimento_cliente($dia_vencimento, $competencia, 'tb2').' AND tb2.status = 1 ';
	...
	public function retornar_faturamento_franquia_consultas_novas_resumido($id_franquia, $inicio, $fim, $dia_vencimento=null, $competencia=null){
		$sql  = 'SELECT tbmain.nome,COUNT(*) AS qtd, ROUND(MAX(custo_franquia),2) AS und, ROUND(SUM(custo_franquia),2) AS custo, ROUND(SUM(custo)) AS custo_interno ';
    ...
    public function retornar_faturamento_franquia_consultas_veicular_resumido($id_franquia, $inicio, $fim, $dia_vencimento=null, $competencia=null){
    ...
    public function retornar_faturamento_franquia_cartas_resumido($id_franquia, $inicio, $fim, $dia_vencimento=null, $competencia=null){
	...
	public function retornar_faturamento_franquia_baixas_resumido($id_franquia, $inicio, $fim, $dia_vencimento=null, $competencia=null){
    ...
    private function sql_filtro_cliente_franquia($id_franquia, $dia_vencimento=null, $competencia=null){
        $sql = 'tb2.id_franquia_fk = '.(int)$id_franquia.' AND tb2.consultor = 0 AND tb2.status = 1';
        if($dia_vencimento!==null){
            $sql .= ' AND '.$this->sql_dia_vencimento_cliente($dia_vencimento, $competencia, 'tb2');
        }
        return $sql;
    }

    private function sql_dia_vencimento_cliente($dia_vencimento, $competencia=null, $alias='cliente'){
        $campo = $alias.'.dia_vencimento';
        $dia_vencimento = (int)$dia_vencimento;
        if($dia_vencimento===30){
            $ultimo_dia = (int)date('t', strtotime(($competencia ? $competencia : date('Y-m')).'-01'));
            return '('.$campo.' = 30 OR '.$campo.' = '.$ultimo_dia.')';
        }
        return $campo.' = '.$dia_vencimento;
    }
