...
class Remessa2_model extends CI_Model{
	...
	public function salvar_remessa_bd($arquivo,$numeracao){
		$content = file_get_contents($arquivo);
		$this->db->insert('remessa',array('numeracao'=>$numeracao,'conteudo'=>$content));
	}

	public function numeracao_ultima_remessa(){
		$sql = 'SELECT numeracao FROM remessa ORDER BY id_remessa DESC LIMIT 1 ';
		$return = $this->db->query($sql);
		if($return!=null){
			$return = $return->row();
			if($return!=null){
				return ($return->numeracao + 1);
			}else{
				return 1;
			}
		}else{
	...
	public function retornar_ultimas($id_franquia = null,$qtd=10){
		$sql = 'SELECT * FROM remessa ORDER BY id_remessa DESC LIMIT '.$qtd;
		return $this->db->query($sql);
	}

	public function retornar($id_franquia, $id_remessa){
		$sql = 'SELECT * FROM remessa WHERE id_remessa = '.$id_remessa;
		return $this->db->query($sql);
	}

	public function iniciar($conta){
		$this->banco = $conta->banco_numero;
		$this->tipo = '2';
		$this->cnpj = adm_env('SICOOB_BENEFICIARIO_CPF_CNPJ', $this->cnpj);
		$this->agencia = $conta->agencia;
		$this->conta = $conta->conta;
		$this->conta_dv = $conta->conta_dv;
		$this->nome_empresa = truncate_boleto($conta->nome_cedente,30);
		$this->nome_banco = strtoupper($conta->banco_nome);
	...
	public function baixar(){
		$hoje = date('Y-m-d');
		$base_file = 'remessas/remessa_rmc_'.str_replace('-','_',$hoje).'.REM';
		$arquivo = FCPATH.$base_file;

		if(file_exists($arquivo)){
			unlink($arquivo);
		}

		$num_atual = $this->numeracao_ultima_remessa();
		$handle = fopen($arquivo,'w') or die('Não foi possível gerar a remessa: '.$arquivo);
		fclose($handle);
		$this->escrever_header($arquivo,$num_atual);
		foreach($this->lotes as $l):
			$this->escrever($arquivo,$l);
		endforeach;
		$this->escrever_trailler($arquivo);

		$this->salvar_remessa_bd($arquivo,$num_atual);

		$file_url = base_url().$base_file;
	...
	public function ecrever_lote_de_boleto($dados_entrada,$linha){
		$dados = new stdClass();
		...
		$this->ecrever_lote($dados,$linha);
	}

	public function ecrever_lote($dados,$linha){
		$retorno  = $this->escrever_lote_header($dados,$linha);
		$retorno .= $this->escrever_lote_registro_p($dados,$linha);
		$retorno .= $this->escrever_lote_registro_q($dados,$linha);
		$retorno .= $this->escrever_lote_registro_r($dados,$linha);
		$retorno .= $this->escrever_lote_registro_s($dados,$linha);
		$retorno .= $this->escrever_lote_trailler($dados,$linha);
		array_push($this->lotes,$retorno);
	}

	private function escrever_lote_header($dados,$auto_incremento_remessa){
		$controle_banco         = $this->banco;
		$lotes                  = $auto_incremento_remessa;
		...
		return $string_para_escrita;
	}

	private function escrever_lote_registro_p($dados,$auto_incremento_remessa){
		$controle_banco         = $this->banco;
		$lotes                  = $auto_incremento_remessa;
		$controle_lote          = $this->pad_left($lotes,4);
		$controle_registro      = '3';
	...
	private function escrever_lote_registro_q($dados,$auto_incremento_remessa){
		$controle_banco         = $this->banco;
		$lotes                  = $auto_incremento_remessa;
	...
	private function escrever_lote_registro_r($dados,$auto_incremento_remessa){
		$controle_banco         = $this->banco;
		$lotes                  = $auto_incremento_remessa;
		$controle_lote_servico  = $this->pad_left($lotes,4);
		$controle_tipo_registro = '3';
	...
	private function escrever_lote_registro_s($dados,$auto_incremento_remessa){
		$controle_banco         = $this->banco;
		$lotes                  = $auto_incremento_remessa;
		$controle_lote_servico  = $this->pad_left($lotes,4);
		$controle_tipo_registro = '3';
	...
	private function escrever_lote_trailler($dados,$auto_incremento_remessa){
		$controle_banco         = $this->banco;
		$lotes                  = $auto_incremento_remessa;
		$controle_lote_servico  = $this->pad_left($lotes,4);
		...
		$CNAB                   = $this->em_branco(9);
		$qtd_registros          = $this->pad_left($this->qtd_linhas(),6);
	...
	public function escrever_trailler($arquivo){
		$controle_banco         = $this->banco;
		$controle_lote          = '9999';
		$controle_registro      = '9';

		$CNAB                   = $this->em_branco(9);
		$qtd_lotes              = $this->pad_left(count($this->lotes),6);
		$qtd_registros          = $this->pad_left($this->qtd_linhas(),6);
		$qtd_contas             = '000000';
	...
	private function qtd_linhas(){
		return 1 + (count($this->lotes));
	}
}
